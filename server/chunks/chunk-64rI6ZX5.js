import { dO as a, fQ as m$1, dP as u, fS as h, fT as y$1, fU as r$1, c4 as d$1, fV as e, bI as b, bb as s$1, bo as e$1, bp as y$2, br as c } from "./chunk-KFNcxJaF.js";
import { r, n } from "./chunk-pRHVu1Qg.js";
...
const d = /* @__PURE__ */ new Set([102113, 102100, 3857, 3785, 900913]), y = [0, 0];
let _ = class extends a(r(m$1(u))) {
  constructor() {
  ...
  attach() {
    ...
    e2 && (this._tileInfoView = new h(e2), this._fetchQueue = new y$1({ tileInfoView: this._tileInfoView, concurrency: 16, process: (e3, t) => this.fetchTile(e3, t) }), this._tileStrategy = new r$1({ cachePolicy: "keep", resampling: true, acquireTile: (e3) => this.acquireTile(e3), releaseTile: (e3) => this.releaseTile(e3), tileInfoView: this._tileInfoView }), this.addAttachHandles(this._updatingHandles.add(() => [this.layer?.activeLayer?.styleId, this.tileMatrixSet], () => this.doRefresh())), super.attach());
  }
  detach() {
  ...
  supportsSpatialReference(e2) {
    return this.layer.activeLayer.tileMatrixSets?.some((t) => d$1(t.tileInfo?.spatialReference, e2)) ?? false;
  }
  async doRefresh() {
    if (this.attached) {
      if (this.suspended)
  ...
  acquireTile(e2) {
    const t = this._bitmapView.createTile(e2), i = t.bitmap;
  ...
  async fetchTile(e$12, t = {}) {
    const s = "tilemapCache" in this.layer ? this.layer.tilemapCache : null, { signal: r2, resamplingLevel: a2 = 0 } = t;
    if (!s)
    ...
    try {
      ...
      if (b(n$1))
        throw n$1;
      if (a2 < 3) {
        const i = this._tileInfoView.getTileParentId(e$12.id);
        if (i) {
          const s2 = new e(i), r3 = await this.fetchTile(s2, { ...t, resamplingLevel: a2 + 1 });
  ...
  async _enqueueTileFetch(e2) {
    if (!this._fetchQueue.has(e2.key.id)) {
      try {
        ...
        b(s) || s$1.getLogger(this).error(s);
      }
      this.requestUpdate();
    }
  }
  ...
  _getTileMatrixSetBySpatialReference(e2) {
    ...
    if (!e2.tileMatrixSets)
      return null;
    let i = e2.tileMatrixSets.find((e3) => d$1(e3.tileInfo?.spatialReference, t));
    return !i && t.isWebMercator && (i = e2.tileMatrixSets.find((e3) => d.has(e3.tileInfo?.spatialReference.wkid ?? -1))), i;
  }
};
e$1([y$2({ readOnly: true })], _.prototype, "tileMatrixSet", null), _ = e$1([c("esri.views.2d.layers.WMTSLayerView2D")], _);
const w = _;
export {
  w as default
};
