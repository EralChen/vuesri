import { aq as has, i6 as a$1, i7 as $, i8 as e$1, i9 as n$2 } from "./chunk-KFNcxJaF.js";
const s$1 = 128e3;
let n$1 = null, e = null;
async function i() {
  return n$1 || (n$1 = o$1()), n$1;
}
async function o$1() {
  const s2 = has("esri-csp-restrictions") ? await import("./chunk-ehqiEQxK.js").then((t) => t.l) : await import("./chunk-4pJUlbWR.js").then((t) => t.l);
  e = await s2.load({ locateFile: (s3) => a$1(`esri/core/libs/libtess/${s3}`) });
}
function r$1(t, n2) {
...
class c {
  ...
  tessellate(t, e2) {
    r(t), this._canUseThinTessellation && e2.halfWidth < $ && !e2.offset ? this._tessellateThin(t, e2) : this._tessellate(t, e2);
  }
  ...
  _tessellate(i2, r2) {
    ...
    if (i2.length < v)
      return;
    const f = r2.pixelCoordRatio, w = null != r2.capType ? r2.capType : e$1.BUTT, T = null != r2.joinType ? r2.joinType : n$2.MITER, g = null != r2.miterLimit ? Math.min(r2.miterLimit, 4) : 2, m = null != r2.roundLimit ? Math.min(r2.roundLimit, 1.05) : 1.05, V = null != r2.halfWidth ? r2.halfWidth : 2, N = !!r2.textured;
    let E, b, R, U = null;
    ...
    const Q = this._writeVertex, X = this._writeTriangle, C = (t, e2, i3, x2, r3, n2) => {
      const s2 = Q(b, R, D, S, i3, x2, t, e2, r3, n2, q);
    ...
    for (let x2 = 0; x2 < i2.length; ++x2) {
      ...
      if (r3 || (q += M), U = c3 ? u ? i2[1] : null : i2[x2 + 1], U ? (p.x = U.x - b, p.y = U.y - R, B = h(p), p.x /= B, p.y /= B) : (p.x = void 0, p.y = void 0), !u) {
        if (r3) {
          s(L, p), D = L.x, S = L.y, w === e$1.SQUARE && (C(-p.y - p.x, p.x - p.y, p.x, p.y, 0, -1), C(p.y - p.x, -p.x - p.y, p.x, p.y, 0, 1)), w === e$1.ROUND && (C(-p.y - p.x, p.x - p.y, p.x, p.y, -1, -1), C(p.y - p.x, -p.x - p.y, p.x, p.y, -1, 1)), w !== e$1.ROUND && w !== e$1.BUTT || (C(-p.y, p.x, p.x, p.y, 0, -1), C(p.y, -p.x, p.x, p.y, 0, 1));
          continue;
        }
        if (c3) {
          n(L, k), D = L.x, S = L.y, w !== e$1.ROUND && w !== e$1.BUTT || (C(k.y, -k.x, -k.x, -k.y, 0, -1), C(-k.y, k.x, -k.x, -k.y, 0, 1)), w === e$1.SQUARE && (C(k.y - k.x, -k.x - k.y, -k.x, -k.y, 0, -1), C(-k.y - k.x, k.x - k.y, -k.x, -k.y, 0, 1)), w === e$1.ROUND && (C(k.y - k.x, -k.x - k.y, -k.x, -k.y, 1, -1), C(-k.y - k.x, k.x - k.y, -k.x, -k.y, 1, 1));
          continue;
        }
      }
      let d2, v2, Q2 = -l(k, p);
      ...
      switch (X2) {
        ...
        case n$2.BEVEL: {
          ...
          if (v2) {
            ...
            I = this._writeVertex(b, R, D, S, p.x, p.y, r4.x, r4.y, e2.x, e2.y, q), P = this._writeVertex(b, R, D, S, p.x, p.y, _2.x, _2.y, i3.x, i3.y, q), this._writeTriangle(l2, t2, P);
          } else {
            if (N) {
              const t2 = this._bevelMiddle;
              t2.x = (h2.x + _2.x) / 2, t2.y = (h2.y + _2.y) / 2, a(O, t2, -k.x, -k.y), C(t2.x, t2.y, -k.x, -k.y, O.x, O.y), a(O, t2, p.x, p.y), I = this._writeVertex(b, R, D, S, p.x, p.y, t2.x, t2.y, O.x, O.y, q), P = this._writeVertex(b, R, D, S, p.x, p.y, r4.x, r4.y, e2.x, e2.y, q);
            } else {
              const t2 = I;
              I = P, P = t2;
            }
        ...
        case n$2.ROUND: {
          ...
          const g2 = o(this._outer, x3), m2 = this._writeVertex(b, R, D, S, p.x, p.y, u2.x, u2.y, i3.x, i3.y, q);
          let V2, E2;
          const U2 = d2 > 2;
          if (U2) {
            let e3;
